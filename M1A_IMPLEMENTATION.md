...
The host libraries from migration tasks 4–5 now include protected patch and choice-question review, durable permission/input decisions, and bounded native history inspection. This does not complete M1a or create a desktop application. OpenCode's functional source remains unchanged.
...
| Protected artifacts | AES-256-GCM encrypted SQLite records in an explicitly supplied private directory outside registered workspaces. The host supplies a 32-byte key; no key provisioning or OS vault integration ships here. |
...
- Desktop presentation for protected patches/questions, accessibility and explicit host key provisioning. Protected review and fixed-choice input are host APIs; no approval/question UI or OS vault integration ships yet.
