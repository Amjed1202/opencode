...
Current progress: the local host, admission, SQLite command/permission/input journals and pinned Codex adapter are implemented and exercised with spawned protocol peers. Bounded once-only file decisions, blocking fixed-choice input, encrypted patch/question review, expiry, native history inspection and exact-turn reconciliation are available through host APIs. Grants and answers require a current actor-bound review token and final authorization at the native write boundary. RC2 also passes a bounded real Codex repository task with denial, exact approved edit, interruption and resume; its restricted execution scope is recorded in [V1 acceptance](V1_STATUS.md). M1a is **not complete**: broader action/raw-artifact handling and OS boundary verification remain open. The [desktop](DESKTOP.md) supplies protected review presentation and OS-protected review keys. See [implementation status](M1A_IMPLEMENTATION.md).
