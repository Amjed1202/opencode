...
Implement the permitted unmodified Claude Code route with native sign-in, structured stream and native permission host. Keep direct SDK subscription use conditional on current official permission; explicit API mode uses its own route. Verify status and stream schemas on installed versions, including Windows process cancellation. Implement OpenCode via one pinned supported API generation, including its own native permissions/history, with capabilities limited to proven functions.
...
Build the M1a admission and Codex slice. Codex is installed in the reconnaissance environment; Claude was not found on PATH. First implement and test **billing-safe preflight**, then event/permission translation; do not start by restyling the whole OpenCode UI. Recheck current provider terms and versioned native contracts at integration time.
