...
Recognized SDK error categories on bound main-assistant `<synthetic>` frames are projected as fixed errors, with an allowlisted native code. Synthetic text, tools and usage are never projected as a response from the chosen model. A synthetic `stop_reason` does not prove completion: the native process is stopped and the input remains uncertain. Live notices and saved activity show fixed error labels, and the host drains newly committed final events before ending a subscription.
...
Provider-free fixtures cover the inspector, execution admission, protected permissions, stale epochs and files, input identity, model identity, multi-block text, clean exact resume, Skills, and actual official SDK framing with a compiled local peer. The installed binary passed no-prompt subscription/control initialization and explicit staged Skill discovery. After native sign-in was restored, one live Haiku input was acknowledged, then rejected by the provider's weekly rate limit before any tool use. The fixture and prepared Skill remained unchanged. Live permission, Skill, interruption and resume acceptance remain pending. [Sanitized first-run evidence](docs/validation/v1-claude-live-rate-limit.json), [current prompt limits](V1_STATUS.md).
