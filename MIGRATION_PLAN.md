...
## 6. Claude and OpenCode vertical slices — restricted Claude candidate

Claude Skills discovery and explicit standalone native activation are implemented with root/hash/invocation/policy bindings. [CLAUDE_SKILLS.md](CLAUDE_SKILLS.md) defines the tested subset and broader exclusions.
...
`packages/harness-desktop` provides separate Electron main, sandboxed preload, Solid renderer and a private Bun worker. It now supports native Codex/Claude selection, subscription/model admission, protected review, saved conversations/recovery, files/diffs, tokens/context capacity and standalone native Claude Skills. One conversation can be attached at a time; configuration changes invalidate prior evidence. OpenCode remains unavailable. The Windows portable candidate bundles Electron/Bun under distinct application data/IDs and a local custom origin, with no updater, share or telemetry destination inherited from upstream. [DESKTOP.md](DESKTOP.md), [V1 acceptance](V1_STATUS.md).

Future installers and updates need owned signing/update channels and coexistence checks. The current portable package has a separate data directory and explicit bundled runtime, with hash and launch verification; it does not implement an installer or updater.

## 8. Follow-on work

The immediate remaining acceptance is the bounded live repository task on both native subscriptions with explicit unknown-overage acknowledgment. OS execution boundary attestation, cross-process fencing and OpenCode execution remain separate foundation work.
