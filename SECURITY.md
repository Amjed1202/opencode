...
Status: native Codex and restricted Claude admission, protected interactions, encrypted review storage and the Electron/Bun desktop are implemented, 2026-09-07. Review keys use OS secure storage. Live provider acceptance, OS boundary attestation, cross-process fencing and remote transport remain open; see [V1 scope](V1_STATUS.md), [Claude controls](V1_CLAUDE.md) and [desktop boundaries](DESKTOP.md). Harness is a provisional codename. The original OpenCode policy is preserved after this section and applies to upstream OpenCode, not to new Harness security guarantees or reporting ownership.
...
The Claude 2.1.251 adapter uses the official SDK with the explicitly selected unmodified native executable. Fixed version/auth diagnostics, personal Pro/Max provider/account observations, unmanaged policy checks and a no-prompt SDK control handshake establish the supported conditional path. Account identifiers are hashed; raw auth fields and diagnostics are discarded. Restricted native settings and a PreToolUse permission hook constrain the native tools to ordinary workspace Read/Edit/Write and explicitly hash-bound standalone Skills. A protected once-only write requires the host's current authority at the SDK stdin allow-reply boundary. Source/operation/policy changes, stale turns and cancellation revoke pending authority. Neither native tool mediation nor successful direct-child termination attests OS sandboxing or descendant isolation. Unconfirmed cleanup rejects the stop proof. Switching runtimes clears the executable, native home and connection evidence, and requires detaching the conversation. [Native evidence](docs/validation/v1-claude-native-evidence.md).
