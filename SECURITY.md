...
Status: host admission, bound native interactions, encrypted review storage, a restricted Electron/Bun desktop and native Claude connection checks are implemented, 2026-09-07. The desktop provisions review keys through OS secure storage. Claude execution, OS execution boundary attestation, remote transport and live provider acceptance remain open; see [Claude scope](M1B_CLAUDE.md), [desktop boundaries](DESKTOP.md) and [host scope](M1A_IMPLEMENTATION.md). Harness is a provisional codename. The original OpenCode policy is preserved after this section and applies to upstream OpenCode, not to new Harness security guarantees or reporting ownership.
...
The Claude 2.1.251 adapter runs only fixed `--version` and `auth status` commands with closed stdin, bounded output, a timeout and bounded cleanup. It uses the selected native account home and a private host working directory, with an explicit environment that excludes credential and provider overrides. Only the native `loggedIn` boolean leaves the inspector; raw account fields, paths and diagnostics are discarded. Sign-in does not establish subscription billing or effective native policy: all execution capabilities and Skills activation remain unsupported, and admission is blocked. Cleanup closes owned streams and attempts to terminate the direct child; it does not attest descendant termination. An unconfirmed direct-child shutdown disables further inspection. Switching runtimes clears the executable, native home and prior connection evidence, and is prohibited with an attached session. [Native evidence and remaining gates](docs/validation/m1b-claude-native-evidence.md)
